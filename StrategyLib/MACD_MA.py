...
LastEditTime: 2022-07-05 21:00:10
LastEditors: adolf
'''
from finta import TA
from BackTrader.base_back_trader import TradeStructure
from StrategyLib.config import macd_ma_config
...
class MACDMA60(TradeStructure):
    ...
    def cal_technical_indicators(self, indicators_config):
        ...
        # 获取到MACD金叉点和死叉点
        self.data.loc[(self.data['HISTOGRAM']>0)&(self.data['HISTOGRAM'].shift(1)<0),'golden'] = "LONG"
        self.data.loc[(self.data['HISTOGRAM']<0)&(self.data['HISTOGRAM'].shift(1)>0),'golden'] = "SHORT"

        self.data['state'] = 0
        self.data.loc[(self.data['HISTOGRAM']>0)&(self.data['close']>self.data["MA60"]),'state'] = 1
    ...
    def trading_algorithm(self):
        ...
        for index,row in self.data.iterrows():
            ...
            if row['golden'] == "LONG":
                ...
                golden = "SHORT"
            if golden == "LONG" and row['state'] == 1 and row['close'] < row['open'] and buy_flag > 0:
