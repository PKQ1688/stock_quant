...
st.text(f"达成目标自然日天数:{natual_day}，投入次数/总金额:{len(res[res['put']!=0])}/{int(res.loc[len(res)-1,'put_in'])}，总收益:{int(res.loc[len(res)-1,'account']-res.loc[len(res)-1,'put_in'])}，收益率:{round(res.loc[len(res)-1,'rate'],3)},标的涨跌幅:{stock_data.loc[len(stock_data)-1,'close']/stock_data.loc[0,'open']}")
