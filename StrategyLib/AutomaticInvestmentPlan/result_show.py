...
if_intelli = st.sidebar.text_input("if_intelli", value="yes")=="yes"

res,stock_data = get_AI_plan_result(code=code,
                         gap_days=int(gap_days), 
                         first_buy_day=first_buy_day, 
                         want_rate=float(want_rate),if_intelli=if_intelli)
res.drop(["buy_index"], axis=1, inplace=True)
st.dataframe(res)
natual_day=getDays(res.loc[0,'date'],res.loc[len(res)-1,'date'])
st.text(f"达成目标自然日天数:{natual_day}，投入次数/总金额:{int(res.loc[len(res)-1,'put_in']/1000)}/{res.loc[len(res)-1,'put_in']}，总收益:{int(res.loc[len(res)-1,'account'])-res.loc[len(res)-1,'put_in']}，收益率:{round(res.loc[len(res)-1,'rate'],3)},标的涨跌幅:{stock_data.loc[len(stock_data)-1,'close']/stock_data.loc[0,'open']}")
