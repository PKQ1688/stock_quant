...
def cal_linear_regression(board_name):
    df = pd.read_csv(board_data_path +
                     "industry_origin/{}.csv".format(board_name))
...
def get_all_data():
    ...
    df_merged.to_csv(board_data_path + "/ALL_INDUSTRY_BOARD_HISTORY.csv",
                     index=False)


def choose_what_need(all_df):
    for board_name in board_dict.keys():
        all_df['{}_pct'.format(board_name)] = all_df['{}_close'.format(
            board_name)] / all_df['{}_close'.format(board_name)].shift(1) - 1

    for index, row in all_df.iterrows():
        # logger.debug(row)
        tmp_mom = row[[
            '{}_mom'.format(board_name) for board_name in board_list
