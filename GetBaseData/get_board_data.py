...
LastEditTime: 2022-07-24 13:41:58
LastEditors: adolf
'''
import os
import json
...
industry_board_name_mapping = stock_board_industry_name_em_df.set_index(['板块名称'])['板块代码'].to_dict()
...
for key,value in tqdm(industry_board_name_mapping.items()):
    # print(key,value)
    stock_board_industry_hist_em_df = ak.stock_board_industry_hist_em(symbol=key, start_date="19900101", end_date="20991231", adjust="")
    stock_board_industry_hist_em_df.rename(columns=ch_eng_mapping_dict, inplace=True)
    # print(stock_board_industry_hist_em_df)
    
    stock_board_industry_hist_em_df.to_csv(save_path+"{}.csv".format(key), index=False)
