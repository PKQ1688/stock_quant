...
def get_stock_data(
    ...
    diff = data_df["MACD"].tolist()
    dea = data_df["SIGNAL"].tolist()
    ...
    for history in db["play_records"].find(filter).limit(count):
        ...
        return_records.append(
            [
                ...
                history["date"],
                history["stock_code"],
                float_to_pct(history["stock_profit_rate"]),
                float_to_pct(history["profit_rate"]),
                float_to_pct(history["over_profit"]),
            ]
        )
    return {
        "records": return_records,
        "profit_rate": profit_rate - 1,
        "stock_profit_rate": stock_profit_rate - 1,
    }
